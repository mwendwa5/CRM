...
use \Exception;
use ChurchCRM\members\FamilyCustomMaster as ChildFamilyCustomMaster;
...
abstract class FamilyCustomMasterQuery extends ModelCriteria
{
    ...
    /**
     * Initializes internal state of \ChurchCRM\members\Base\FamilyCustomMasterQuery object.
     *
     * @param     string $dbName The database name
     * @param     string $modelName The phpName of a model, e.g. 'Book'
     * @param     string $modelAlias The alias for the model in this query, e.g. 'b'
     */
    public function __construct($dbName = 'churchcrm', $modelName = '\\ChurchCRM\\members\\FamilyCustomMaster', $modelAlias = null)
    {
     ...
     * $query->filterByCustomField('%fooValue%'); // WHERE fam_custom_Field LIKE '%fooValue%'
     * </code>
     *
     ...
     * $query->filterByCustomName('%fooValue%'); // WHERE fam_custom_Name LIKE '%fooValue%'
     * </code>
     *
     ...
     * $query->filterByCustomSide('%fooValue%'); // WHERE fam_custom_Side LIKE '%fooValue%'
     * </code>
     *
    ...
    public function filterByCustomSide($customSide = null, $comparison = null)
    {
        if (null === $comparison) {
            if (is_array($customSide)) {
                $comparison = Criteria::IN;
            } elseif (preg_match('/[\%\*]/', $customSide)) {
                $customSide = str_replace('*', '%', $customSide);
                $comparison = Criteria::LIKE;
            }
        }
