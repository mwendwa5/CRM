...
namespace ChurchCRM\members\Base;

use \Exception;
use \PDO;
use ChurchCRM\members\Family as ChildFamily;
use ChurchCRM\members\FamilyQuery as ChildFamilyQuery;
use ChurchCRM\members\Map\FamilyTableMap;
...
abstract class FamilyQuery extends ModelCriteria
{
    ...
    /**
     * Initializes internal state of \ChurchCRM\members\Base\FamilyQuery object.
     *
     * @param     string $dbName The database name
     * @param     string $modelName The phpName of a model, e.g. 'Book'
     * @param     string $modelAlias The alias for the model in this query, e.g. 'b'
     */
    public function __construct($dbName = 'churchcrm', $modelName = '\\ChurchCRM\\members\\Family', $modelAlias = null)
    {
    ...
    public function findPk($key, ConnectionInterface $con = null)
    {
        ...
        if ((null !== ($obj = FamilyTableMap::getInstanceFromPool(null === $key || is_scalar($key) || is_callable([$key, '__toString']) ? (string) $key : $key))) && !$this->formatter) {
     ...
     * $query->filterByName('%fooValue%'); // WHERE fam_Name LIKE '%fooValue%'
     * </code>
     *
     ...
     * $query->filterByAddress1('%fooValue%'); // WHERE fam_Address1 LIKE '%fooValue%'
     * </code>
     *
     ...
     * $query->filterByAddress2('%fooValue%'); // WHERE fam_Address2 LIKE '%fooValue%'
     * </code>
     *
     ...
     * $query->filterByCity('%fooValue%'); // WHERE fam_City LIKE '%fooValue%'
     * </code>
     *
     ...
     * $query->filterByState('%fooValue%'); // WHERE fam_State LIKE '%fooValue%'
     * </code>
     *
     ...
     * $query->filterByZip('%fooValue%'); // WHERE fam_Zip LIKE '%fooValue%'
     * </code>
     *
     ...
     * $query->filterByCountry('%fooValue%'); // WHERE fam_Country LIKE '%fooValue%'
     * </code>
     *
     ...
     * $query->filterByHomephone('%fooValue%'); // WHERE fam_HomePhone LIKE '%fooValue%'
     * </code>
     *
     ...
     * $query->filterByWorkphone('%fooValue%'); // WHERE fam_WorkPhone LIKE '%fooValue%'
     * </code>
     *
     ...
     * $query->filterByCellphone('%fooValue%'); // WHERE fam_CellPhone LIKE '%fooValue%'
     * </code>
     *
     ...
     * $query->filterByEmail('%fooValue%'); // WHERE fam_Email LIKE '%fooValue%'
     * </code>
     *
     ...
     * $query->filterByScanCheck('%fooValue%'); // WHERE fam_scanCheck LIKE '%fooValue%'
     * </code>
     *
    ...
    public function filterByScanCheck($scanCheck = null, $comparison = null)
    {
        if (null === $comparison) {
            if (is_array($scanCheck)) {
                $comparison = Criteria::IN;
            } elseif (preg_match('/[\%\*]/', $scanCheck)) {
                $scanCheck = str_replace('*', '%', $scanCheck);
                $comparison = Criteria::LIKE;
            }
        }
     ...
     * $query->filterByScanCredit('%fooValue%'); // WHERE fam_scanCredit LIKE '%fooValue%'
     * </code>
     *
    ...
    public function filterByScanCredit($scanCredit = null, $comparison = null)
    {
        if (null === $comparison) {
            if (is_array($scanCredit)) {
                $comparison = Criteria::IN;
            } elseif (preg_match('/[\%\*]/', $scanCredit)) {
                $scanCredit = str_replace('*', '%', $scanCredit);
                $comparison = Criteria::LIKE;
            }
        }
     ...
     * $query->filterBySendNewsletter('%fooValue%'); // WHERE fam_SendNewsLetter LIKE '%fooValue%'
     * </code>
     *
    ...
    public function filterBySendNewsletter($sendNewsletter = null, $comparison = null)
    {
        if (null === $comparison) {
            if (is_array($sendNewsletter)) {
                $comparison = Criteria::IN;
            } elseif (preg_match('/[\%\*]/', $sendNewsletter)) {
                $sendNewsletter = str_replace('*', '%', $sendNewsletter);
                $comparison = Criteria::LIKE;
            }
        }
     ...
     * $query->filterByOkToCanvass('%fooValue%'); // WHERE fam_OkToCanvass LIKE '%fooValue%'
     * </code>
     *
    ...
    public function filterByOkToCanvass($okToCanvass = null, $comparison = null)
    {
        if (null === $comparison) {
            if (is_array($okToCanvass)) {
                $comparison = Criteria::IN;
            } elseif (preg_match('/[\%\*]/', $okToCanvass)) {
                $okToCanvass = str_replace('*', '%', $okToCanvass);
                $comparison = Criteria::LIKE;
            }
        }
